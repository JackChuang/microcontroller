...
int main(void)
{
    ...
    CS_setDCOFrequency(MY_DCO_FREQUENCY_12);
...
//        ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ENTER CODE FOR LAB3 stopwatch init HERE:
    ...
    MAP_Timer32_setCount(TIMER32_0_BASE, MY_DCO_FREQUENCY_12/1000);
    MAP_Timer32_enableInterrupt(TIMER32_0_BASE);
    start_watch_timer();

    // buttons (IMPORTANT P1.1 IS USED BY UART IN THIS LAB AS WELL!!!!!!!!!!!)
...
void show_time(void)
{
    printf(EUSCI_A0_BASE, "Current time =  %i ms running = %s\r\n",
           curr_t, running?"(O)":"(X)");
}
